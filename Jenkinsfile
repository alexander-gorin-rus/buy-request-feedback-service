...
pipeline {
  ...
  environment {
     APP_NAME = 'BuyRequest/feedback-service'
     WORKSPACE = '/home/Jenkins/workspace/BuyRequest/feedback-service'
  }
   agent none
   stages {
            ...
            steps {
                script {
                    sshagent (credentials: ['buyrequestdev']) {
                        ...
                        && cat .env \
                        && cd ../services/feedback-service && git checkout -f && git pull \
                        && cd /home/ubuntu/buy-request/docker && make feedback-service-restart && make nginx-restart \
                        && make feedback-service-logs-without-tail \
